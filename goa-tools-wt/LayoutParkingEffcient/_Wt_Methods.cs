...
namespace wt_Common
{
    ...
    public class _Methods
    {
        ...
        #region 将线条临时显示在界面上
        public static void ShowTempGometry(Document doc, List<Line> _lines)
        {
        ...
        #region 判断多根线，或者，两根线是否首位相连 或 闭合
        /// <summary>
        /// 判断所有直线是否首位相连，该函数只能判断出每个线段的端点被重合一次，是不是属于曲线闭合需要与iscurveloop函数一起使用
        /// </summary>
        /// <param name="_ModelLines"></param>
        /// <returns></returns>
        public static bool _LinesIsLoop(IList<Line> _Lines)
        {
            int count = _Lines.Count * 2;//被判断交点的数量
            ...
            foreach (Line _Line in _Lines)
            {
                ...
                foreach (Line __Line in _Lines)
                {
                    if (!_Line.Equals(__Line))
                    {
                        XYZ __endpoing_0 = __Line.GetEndPoint(0);
                        XYZ __endpoing_1 = __Line.GetEndPoint(1);

                        double tolerance = 0.000001;//容差在千分之一内点的就算重合
        ...
        /// <returns></returns>
        public static XYZ GetLeftDownXYZfromLines(IList<Line> _Lines, out double distance_x, out double distance_y)
        {
            //不可以用 0 作为首次被比选数据
        ...
        /// <param name="distance_y"></param>
        /// <returns></returns>
        public static XYZ GetLeftUpXYZfromLines(IList<Line> _Lines, out double distance_x, out double distance_y)
        {
            //不可以用 0 作为首次被比选数据
            Line _Line_0 = _Lines[0];
            double _X_min = _Line_0.GetEndPoint(0).X;
        ...
        /// <param name="distance_y"></param>
        /// <returns></returns>
        public static XYZ GetRigthUpXYZfromLines(IList<Line> _Lines, out double distance_x, out double distance_y)
        {
            //不可以用 0 作为首次被比选数据
            Line _Line_0 = _Lines[0];
            double _X_min = _Line_0.GetEndPoint(0).X;
        ...
        /// <param name="distance_y"></param>
        /// <returns></returns>
        public static XYZ GetRightDownXYZfromLines(IList<Line> _Lines, out double distance_x, out double distance_y)
        {
            //不可以用 0 作为首次被比选数据
            Line _Line_0 = _Lines[0];
            double _X_min = _Line_0.GetEndPoint(0).X;
        ...
        /// <param name="_Lines"></param>
        /// <returns></returns>
        public static double GetAreafromLines(List<Line> _Lines)
        {
            //获取线段断点列表，并去重
        ...
        public static void SetLineStyle(Document doc, string _tarLineStyleName, CurveElement _CurveElement)
        {
        ...
        /// <param name="_Lines"></param>
        /// <returns></returns>
        public static List<XYZ> GetUniqueXYZFromLines(List<Line> _Lines)
        {
        ...
        {
            List<Line> _Lines = new List<Line>();
            Document doc = LineGroup.Document;
            List<ElementId> _groupEleIds = LineGroup.GetMemberIds().ToList();
        ...
        /// <param name="_XYZs"></param>
        /// <returns></returns>
        public static List<XYZ> Pointlistdeduplication(List<XYZ> _XYZs)
        {
            //列表去重
        ...
        public static bool IsInXYZlist(XYZ _XYZ, IList<XYZ> _XYZs)
        {
            ...
            foreach (XYZ __XYZ in _XYZs)
            {
                ...
                if (_distance < 0.003)//该处需要注意 Revit2020版本中 曲线的最小容差为 0.00256026455729167 Feet 0.7803686370625 MilliMeter
