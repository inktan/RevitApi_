...
{
    internal class ConBeamMultiline : EleCreatInfo
    {
        /// <summary>
        /// 混凝土梁
        /// </summary>
        public ConBeamMultiline() : base()
        {
        }
        ...
        internal static IEnumerable<MlineInfo> LinePair(List<LineInfo> lineInfos)
        {
            ...
            foreach (var lineInfo01 in lineInfos)
            {
                ...
                //配对处理，找到与基准线相邻最近的平行，得到定位梁位置的两根梁线
        ...
        static public void LineToMul(MlineInfo mlineInfo)
        {
            ...
            Segment2d result;
            //两根梁线，用哪一根梁线
            //当前遇到问题，两根定位梁线，有碎数的情况
            if (seg01.Length > seg02.Length)// 留长边，长边未必是整数，要选择是整数的梁线
            {
            ...
            double seg01_d = seg01.Length.FeetToMilliMeter() + 0.00001;
            double seg02_d = seg02.Length.FeetToMilliMeter() + 0.00001;

            if (Math.Abs(seg01_d % 1) < 0.0005 && Math.Abs(seg02_d % 1) > 0.0005)
            {
                Vector2d direction01 = seg01.Direction.Rotate(Vector2d.Zero, Math.PI * 0.5 * (-1));
                Vector2d direction02 = seg01.Direction.Rotate(Vector2d.Zero, Math.PI * 0.5);
            ...
            else if (Math.Abs(seg01_d % 1) > 0.0005 && Math.Abs(seg02_d % 1) < 0.0005)
            {
                Vector2d direction01 = seg02.Direction.Rotate(Vector2d.Zero, Math.PI * 0.5 * (-1));
                Vector2d direction02 = seg02.Direction.Rotate(Vector2d.Zero, Math.PI * 0.5);
