...
namespace TOPO_ANLS
{
    ...
    public class CMD_elevation2 : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
           ...
           //   TaskDialog.Show("goa", planes_up.Count.ToString());
            ...
            double soliderrcount = 0;
            double newsoliderrcount = 0;
      ...
      //              if (normal.Z < 0) { normal = normal.Negate(); }
                    ...
                    for (int j = 0; j < planes_down.Count; j++)
                    {
                        if(solid == null) { soliderrcount++; break; }
                        Solid solid_cut = BooleanOperationsUtils.CutWithHalfSpace(solid, planes_down[j]);
                        if (solid_cut == null) { newsoliderrcount++; continue; }
                        
                        Face face = null;
                        foreach (Face temp in solid_cut.Faces)
                        {
                            PlanarFace pf = temp as PlanarFace;
                            if (pf == null) { continue; }
                            if (pf.FaceNormal.IsAlmostEqualTo(normal) || pf.FaceNormal.IsAlmostEqualTo(normal.Negate())) { face = temp; break; }
                        }
                        ...
                        //如果solid在两个切面中间,没有与任何一个切面相切,那么就直接break出去.
                        if (solid.Volume == solid_cut.Volume) { break; }

                        BooleanOperationsUtils.CutWithHalfSpaceModifyingOriginalSolid(solid, planes_up[j]);
                    }
            ...
            TaskDialog.Show("soliderr", soliderrcount.ToString());

            TaskDialog.Show("newsoliderr", newsoliderrcount.ToString());
