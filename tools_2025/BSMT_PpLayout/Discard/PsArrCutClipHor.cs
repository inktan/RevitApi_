...
//    /// 阵列式停车位布局 - 水平式
//    /// </summary>
//    class PsArrCutClipHor : PsCutClipPattern
//    {
//        /// <summary>    
//        /// 阵列式停车位布局 - 水平式
//        /// </summary>
//        internal PsArrCutClipHor()
//        {        }
//        /// <summary>
...
//        /// </summary>
//        internal List<PPLocPoint> SweepRecursion(Vector2d startLDPoint, Polygon2d polygon2d, List<Polygon2d> obstacleOs, List<Segment2d> obstacleSegs, int sweepRowCount, int parkingCount, double rotateAngle)
//        {
//            Polygon2d circularPolygon2d = polygon2d;//更新每次剩余线圈的空间范围
//            Vector2d moveHeidht_leftDownVector2d = startLDPoint;// 用于停车位定位点的高度数据更新，基于sweepRowCount与parkingCount判断行数与列数
...
//                if (Math.Abs(newVector2d.y - startLDPoint.y - this.Height) <= Precision_.Precison)// 向上移动一行，行高为停车位高度
//                {
//                    sweepRowCount += 1;
//                    parkingCount = 0;
//                    moveHeidht_leftDownVector2d = moveHeidht_leftDownVector2d + new Vector2d(0, this.Height);// 处理裁剪车位Z轴高度移动问题
//                }
//                else if (Math.Abs(newVector2d.y - startLDPoint.y - this.Height - this.RoadWidth) <= Precision_.Precison)// 向上移动一行，行高为停车位高度+道路宽度
//                {
//                    sweepRowCount += 1;
//                    parkingCount = 0;
//                    moveHeidht_leftDownVector2d = moveHeidht_leftDownVector2d + new Vector2d(0, this.Height + this.RoadWidth);
//                }

//                startLDPoint = AddWifhtAndColumnParkingPoint(moveHeidht_leftDownVector2d, this.Width, this.ColumnWidth, this.ColumnBufferDistance, parkingCount);// 基于每一行的最左边起点，计算当前裁剪车位的位置

//                #endregion

//                #region 停车位机器人登场
//                ParkingPP_base parkingPP_Base = new ParkingPP_base(startLDPoint, this.Height, this.Width);// 基础停车位
//                ParkingPP_robot_withRoad parkingPP_robot = new ParkingPP_robot_withRoad(parkingPP_Base, sweepRowCount, this.RoadWidth, parkingCount, this.ColumnWidth, this.ColumnBufferDistance);// 停车位机器人 基于行列关系，判断柱子和车道的空间
//                bool collision02 = parkingPP_robot.Collision(obstacleOs);// 车位是否被障碍物线圈包含
