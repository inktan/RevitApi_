...
{
    class RetainWallAnakysis_Internal : RequestMethod
    {
        internal RetainWallAnakysis_Internal(UIApplication _uiApp) : base(_uiApp)
        {
        ...
        void Computer(List<Element> elements, Tri3dOctree tri3DOctree)
        {
            ...
            // 两两配对
            List<AdjacentFloor> adjacentFloors = new List<AdjacentFloor>();
            for (int i = 0; i < singleSubPlotEarthworks.Count; i++)
            {
                for (int j = 0; j < singleSubPlotEarthworks.Count; j++)
                {
                    if (j > i)
                    {
                        Polygon2d polygon2d01 = singleSubPlotEarthworks[i].poly2DSampling.polygon2d;
                        Polygon2d polygon2d02 = singleSubPlotEarthworks[j].poly2DSampling.polygon2d;
                        if (polygon2d01.OutwardOffeet(1.0).Intersects(polygon2d02.OutwardOffeet(1.0)))
                        {
                            AdjacentFloor adjacentFloor = new AdjacentFloor(singleSubPlotEarthworks[i], singleSubPlotEarthworks[j]);
                            adjacentFloors.Add(adjacentFloor);
                        }
                    }
                }
            }
            if (adjacentFloors.Count < 1) return;
            foreach (var item in adjacentFloors)
            {
                item.CalOverlaoBoundary();
                List<Segment2d> segs = item.OverlapSegs;
                double elevationDifference = item.EleDifference;

                // 处理挡土墙统计问题
                //CMD.Doc.CreateDirectShapeWithNewTransaction(segs.ToLines());
                //segs.Count.ToString().TaskDialogErrorMessage();
            }

            DataPartitionStatistics(adjacentFloors);

            this.doc.SaveCsv(Variable_.CsvInfo, "挡土墙-内部-高度计算");

            string filePath = "挡土墙高度计算完成，数据存放在" + doc.PathName;
            int length = filePath.Length;
            filePath = filePath.Substring(0, length - 4) + "挡土墙-内部-高度计算" + @".csv";
        ...
        void DataPartitionStatistics(List<AdjacentFloor> adjacentFloors)
        {
            ...
            // 对各个面域的数据进行统计
            adjacentFloors.ForEach(item =>
            {
                if (item.EleDifference > 0.0)
                {
                    triangleInfos.AddRange(item.triangleInfos);
                    if (item.EleDifference > 0 && item.EleDifference < 3000.0.MilliMeterToFeet())
                    {
                        length01 += item.Length;
                    }
                    else if (item.EleDifference >= 3000.0.MilliMeterToFeet() && item.EleDifference < 6000.0.MilliMeterToFeet())
                    {
                        length02 += item.Length;
                    }
                    else if (item.EleDifference >= 6000.0.MilliMeterToFeet() && item.EleDifference < 8000.0.MilliMeterToFeet())
                    {
                        length03 += item.Length;
                    }
                    else if (item.EleDifference >= 8000.0.MilliMeterToFeet() && item.EleDifference < 10000.0.MilliMeterToFeet())
                    {
                        length04 += item.Length;
                    }
                    else if (item.EleDifference >= 10000.0.MilliMeterToFeet() && item.EleDifference < 20000.0.MilliMeterToFeet())
                    {
                        length05 += item.Length;
                    }
                }
            });
