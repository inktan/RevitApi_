...
namespace BSMT_PpLayout
{
    ...
    /// </summary>
    class PsArrCutClipLinearAuto_GerV : PsCutClipPattern
    {
        /// <summary>
        /// 自动判定放置什么车位 针对一排车位进行处理
        /// </summary>
        internal PsArrCutClipLinearAuto_GerV()
        { }
        ...
        internal override List<PPLocPoint> Sweep(Polygon2d polygon2d, Polygon2d bsmtPolygon2d, List<Polygon2d> obstacleOs)
        {
            ...
            while (i < 1e4)// 无限循环，子区域裁剪结束，会自动跳出循环
            {
                ...
                if (MeetCollisionConditions)// 符合停车要求
                {
                    ...
                    PPLocPoint tarPPLocPoint = new PPLocPoint(parkingPP_robot.Robot.PlacePoint, this.PsTypeName, GlobalData.Instance.pSWidth_num, GlobalData.Instance.pSHeight_num);//停车位的实际放置点 // 车头在南侧elementId -122543112
