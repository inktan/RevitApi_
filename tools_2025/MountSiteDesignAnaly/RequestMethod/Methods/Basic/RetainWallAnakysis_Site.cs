...
{
    class RetainWallAnakysis_Site : RequestMethod
    {
        internal RetainWallAnakysis_Site(UIApplication _uiApp) : base(_uiApp)
        {
        ...
        internal override void Execute()
        {
            ...
            Floor floor_red_line = new FilteredElementCollector(this.doc)
                .OfCategory(BuiltInCategory.OST_Floors)
                .WhereElementIsNotElementType()
                .Cast<Floor>()
                .Where(p => p.Name.Contains("红线"))
                .FirstOrDefault();
            ...
            // 
            List<Element> elements_red_line = new List<Element>() { floor_red_line };
            Computer(elements_red_line, elements, tri3DOctree);
        }
        ...
        /// 收集所有元素的线圈
        /// </summary>
        void Computer(List<Element> elements_red_line, List<Element> elements, Tri3dOctree tri3DOctree)
        {
            ...
            #region 对所有楼板进行处理，获取场地外边界的挡土墙分析数据
            ...
            #region 处理场地红线
            List<SinglePlot> singlePlots_red_line = new List<SinglePlot>();
            List<SingleSubPlotEarthwork> singleSubPlotEarthworks_red_line = new List<SingleSubPlotEarthwork>();
            List<SingleCellEarthwork> singleCellEarthworks_red_line = new List<SingleCellEarthwork>();

            if (elements_red_line.Count < 1) return;

            foreach (var p0 in elements_red_line)
            {
                CMD.PlotNum++;
                SinglePlot singlePlot = new SinglePlot(p0);
                singlePlots_red_line.Add(singlePlot);
                singlePlot.SolidHeightExtraction();
                ...
                panarFaceCoils.ForEach(p2 =>
                {
                    ...
                    // 这里取的采样点为各个子细胞边界采样点，只是为了计算挡土墙
                    singleSubPlotEarthwork.SplitSelf_RetainWall_Site(allPolygon2ds);

                    singleCellEarthworks_red_line.AddRange(singleSubPlotEarthwork.singleCellEarthworks);
                    singleSubPlotEarthworks_red_line.Add(singleSubPlotEarthwork);
                    singlePlot.singleSubPlotEarthworks.Add(singleSubPlotEarthwork);
                });
            };

            if (singleSubPlotEarthworks_red_line.Count < 1) return;
            if (singleCellEarthworks_red_line.Count < 1) return;

            // 数据并行计算 多线程
            Variable_.CsvInfo.Add(new object[] { "并行计算耗时", DateTime.Now });

            Parallel.ForEach(singleCellEarthworks_red_line, item =>
            {
                item.Computer();
            });
            ...
            #region 红线内可处理的土方空间，与外部挡土墙的关系研究

            var var01 = singleSubPlotEarthworks; // 设计场地外部挡土墙数据
            List<SingleCellEarthwork> var01_ = var01.SelectMany(p => p.singleCellEarthworks).ToList();
            List<TriangleInfo> triangleInfos01 = var01_.SelectMany(p => p.triangleInfos).ToList();

            //CMD.Doc.CreateDirectShapeWithNewTransaction(triangleInfos01.Select(p=>p.Segment3d.ToLine()));

            var var02 = singleSubPlotEarthworks_red_line; // 设计场地外红线内处理空间
            List<SingleCellEarthwork> var02_ = var02.SelectMany(p => p.singleCellEarthworks).ToList();
            List<TriangleInfo> triangleInfos02 = var02_.SelectMany(p => p.triangleInfos).ToList();

            //CMD.Doc.CreateDirectShapeWithNewTransaction(triangleInfos02.Select(p => p.Segment3d.ToLine()));


            //借用八叉树进行提速

            foreach (var tri01 in triangleInfos02)// 判断最近的挡土墙数据
            {
                tri01.Retain_Wall_Elecation_Difference = triangleInfos01.OrderBy(
                    p =>
                    p.Triangle3d
                    .Center()
                    .ToVector2d()
                    .DistanceSquared(tri01.Triangle3d.Center().ToVector2d())).First().Retain_Wall_Elecation_Difference;
            }

            #endregion


            DataPartitionStatistics(singleSubPlotEarthworks_red_line);
            //DataPartitionStatistics(singlePlots);

            this.doc.SaveCsv(Variable_.CsvInfo, "挡土墙-场地-高度计算");
