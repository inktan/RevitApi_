...
{
    internal class IncludingLinkedModelsAll : RequestMethod
    {
        internal IncludingLinkedModelsAll(UIApplication _uiApp) : base(_uiApp)
        {
        }
        ...
        {
            // 获取文档中所有的三维实体元素
            List<Element> elementsToCheck = new FilteredElementCollector(this.doc).WhereElementIsNotElementType()
            ...
            // 获取文档中所有的三维l链接模型
            List<RevitLinkInstance> rvtLinks = new FilteredElementCollector(this.doc).WhereElementIsNotElementType().OfCategory(BuiltInCategory.OST_RvtLinks).Cast<RevitLinkInstance>().ToList();
            //rvtLinks.Count.ToString().TaskDialogErrorMessage();
