...
{
    internal class ConBeamFollowFace : EleCreatInfo
    {
        /// <summary>
        /// 混凝土梁
        /// </summary>
        public ConBeamFollowFace() : base()
        {
        }
        ...
        internal static IEnumerable<MlineInfo> LinePair(List<LineInfo> lineInfos)
        {
            ...
            foreach (var lineInfo01 in lineInfos)
            {
                ...
                // 先找到所有的平行线，并进行碰撞分析
                Polygon2d polygon2d01 = lineInfo01.Segment2d.ToRenct2d(1.5);
                List<LineInfo> parrelLineInfo = new List<LineInfo>();

                foreach (var lineInfo02 in lineInfos)
                {
                    if (lineInfo02.Line == null) continue;
                    if (lineInfo01 == lineInfo02) continue;
                    ...
                    {
                        Polygon2d polygon2d02 = lineInfo02.Segment2d.ToRenct2d(1.5);

                        if (polygon2d01.Intersects(polygon2d02))
                        {
                ...
                LineInfo lineInfo = parrelLineInfo.Where(p => !p.Center.DistanceTo(lineInfo01.Center).EqualZreo()).OrderBy(p => p.Center.DistanceTo(lineInfo01.Center)).FirstOrDefault();
        ...
        internal override void OpenTrans()
        {
            ...
            foreach (var p in this.beamInfos)
            {
                try
                {
                    ...
                    curve = ProjectToRoof(curve, roofFaces);
        ...
        static internal Curve ProjectToRoof(Curve curve, List<Face> roofFaces)
        {
            ...
            Level tmpLevel = levels.OrderBy(p => p.Elevation).FirstOrDefault();

            // 基于基准线创造一个face
            Element wallType = new FilteredElementCollector(CMD.Doc).OfCategory(BuiltInCategory.OST_Walls).OfClass(typeof(WallType)).Cast<WallType>().Where(p => p.Kind == WallKind.Basic).FirstOrDefault();
            if (wallType == null)
            {
            ...
            else
            {
                ...
                using (Transaction trans = new Transaction(CMD.Doc, "---"))
                {
                    ...
                    //将墙体移动墙宽度的一半，用于定位梁的定位中心线
                    ElementTransformUtils.MoveElement(CMD.Doc, wall.Id, transform.OfVector(line.Direction) * (wallType as WallType).Width * 0.5);
                ...
                foreach (var item in faces)
                {
                    ...
                    if (intersection != null && intersection.Distance.EqualZreo())
                    {
                        ...
                        if (tmp is HermiteSpline)
                        {
                            ...
                            tmp = HermiteSpline.Create(hermiteSpline.ControlPoints, true);
                        }
                        else
                        {
                            tmp = tmp.CreateReversed();
                        }
                        using (Transaction trans = new Transaction(CMD.Doc, "---"))
                        {
                            trans.DeleteErrOrWaringTaskDialog();
                            trans.Start();
                            CMD.Doc.Delete(wall.Id);
                            trans.Commit();
                        }
                        return tmp;
                    }
